...
3. 스크립트 실행: python mssql_init.py
"""
...
def main():
    ...
    # 안전하게 URL 표시 (비밀번호 숨김)
    url_display = settings.mssql_database_url
    if '@' in url_display:
        ...
        if len(parts) == 2:
            ...
            if len(auth_part) == 2:
                ...
                if ':' in user_pass:
                    ...
                    url_display = f"{scheme}://{user}:***@{parts[1]}"
    
    print(f"🔗 연결 URL: {url_display}")
    ...
    try:
        ...
        print("\n📝 다음 단계:")
        print("1. .env 파일에서 DATABASE_TYPE=mssql 설정 확인")
        print("2. backend 폴더에서 서버 실행: python main.py")
        print("3. 브라우저에서 접속: http://localhost:8000")
        print("4. 위의 계정 정보로 로그인 테스트")
